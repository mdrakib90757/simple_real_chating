...
class _CallPageState extends State<CallPage>
  ...
  bool _callAcceptedByZego = false;
  ...
  void initState() {
    ...
    if (widget.isCaller) {
      ...
      // If it's the receiver and Zego's UI should take over, this CallPage might not even be pushed.
      // If it is, it might indicate an issue in the navigation flow.
      // For now, let's just pop it if somehow reached.
  ...
  void _listenCallStatus() {
    _callSub = FirebaseFirestore.instance
        ...
        .listen((doc) {
          ...
          // ✅ Receiver accepted → just pop waiting screen
          if (newStatus == "accepted" && !_hasNavigatedToZego) {
            _hasNavigatedToZego = true;

            // Caller can optionally join Zego if needed:
            _joinZegoCall(); // Only if manual join required
  ...
  void _joinZegoCall() {
    ...
    if (currentUser == null) {
      debugPrint("❌ User not logged in. Cannot join call.");
      return;
    }
